...
def test_model_divergence(model_cnn):
    ...
    axs[0].plot(mean_l2[:100],label="No parameterisation",color="blue")
    axs[0].fill_between(np.linspace(0,99,100),mean_l2[:100]+std_l2[:100],mean_l2[:100]-std_l2[:100],alpha=0.1,color="blue")
    axs[0].plot(mean_l2_param[:100],label="Parameterised sim",color="orange")
    axs[0].fill_between(np.linspace(0,99,100),mean_l2_param[:100]+std_l2_param[:100],mean_l2_param[:100]-std_l2_param[:100],alpha=0.1,color="orange")
    axs[0].legend()
    axs[0].set_title("MSE between high-res coarsened sim, and low-res sim")
    axs[0].set_ylabel("MSE")
    axs[0].set_xlabel("Timestep")

    axs[1].plot(mean_l2,label="No parameterisation",color="blue")
    axs[1].fill_between(np.linspace(0,999,1000),mean_l2+std_l2,mean_l2-std_l2,alpha=0.1,color="blue")
    axs[1].plot(mean_l2_param,label="Parameterised sim",color="orange")
    axs[1].fill_between(np.linspace(0,999,1000),mean_l2_param+std_l2_param,mean_l2_param-std_l2_param,alpha=0.1,color="orange")
    axs[1].legend()
    axs[1].set_title("MSE between high-res coarsened sim, and low-res sim")
